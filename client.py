                ...
                if received_data != '':    
                    ...
                    if request_type == 'prep':
                        ...
                        if self.kiosk.ack_counter == self.kiosk.majority_count:
                            ...
                            print 'I am the leader here !!'
                            ##########################HEARTBEAT_SENT_LIST_POPULATED_START###########################################################
                            #this is where i shud start sending heartbeat to tell ppl !! Yo !! I am the leader
                            ###I am telling everyone about me as a leader !
                            for key in self.config.REM_CLIENTS:
                                self.kiosk.send_heartbeat_list.append({'to':key,'from':self.config.client_id,'log':self.kiosk.log})
                            ######################HEARTBEAT_SENT_LIST_POPULATED_END########################################################
                    ...
                    #It is a very important requirement. Take note!
                        ...
                        if self.kiosk.lastAuthRcvTime==-1:
                            print 'He will be hearing from leader now..'+received_data_json['senderID']
                            self.kiosk.CURRENT_LEADER=received_data_json['senderID']
                            self.kiosk.lastAuthRcvTime=brandnewTime #He will be hearing from leader now
                        else:
...
class SystemConfig():
    def __init__(self):
        ...
        with open('config.json','r') as data_file:    
            ...
            self.timeout_delay=data['process_dict'][self.client_id]['timeout']
    ...
    def sendData(self):
        ...
        while True:
            ...
            while len(self.kiosk.send_ack_accept_list) > 0:
                ...
                #print len(self.kiosk.send_ack_accept_list)
                try:
            ...
            counter=0;
            while len(self.kiosk.send_heartbeat_list) > 0:
                try:
                    if counter>(len(self.kiosk.send_heartbeat_list)-1):
                        counter=0
                    chan=self.kiosk.send_heartbeat_list[counter]
                    to_val=chan['to']
                    #print '@@@@@@@@@@@@Sending heartbeat@@@@@@@@@@@@@@@@@@ !'+ to_val
                    from_val=chan['from']
                    print 'Sending heartbeat auth to: '+to_val +'from our leader..'+ from_val
                    to_send = json.dumps({'senderID': from_val, 'log':chan['log'], 'type': 'leaderAuth' })
                    self.config.send_channels[to_val].send(to_send)
                    counter=counter+1
                    time.sleep(0.5)###inside for loop again cocatination of msg while sending 
                except Exception as e:
                    print 'Error while sending heartbeat' + str(e)
                    print 'Try with the next item in loop'
                    counter=counter+1
